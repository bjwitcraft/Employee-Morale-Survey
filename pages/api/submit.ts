...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const metaIn = parsed.data.meta || {};
  const shiftCode = String(metaIn.shift || '').toUpperCase() as keyof typeof SHIFTS;
  const shiftInfo = SHIFTS[shiftCode];

  const doc: any = {
    createdAt: new Date().toISOString(),
    meta: { ...metaIn, shiftInfo, ipHash: hash(`${ip}|${ua}`).toString(16) },
