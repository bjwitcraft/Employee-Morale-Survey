...
export default function Admin(){
  ...
  async function load(){
    setLoading(true);
    const res = await fetch(`/api/admin/metrics?${qs({range, team, shift, location, category})}`);
    setLoading(false);
  ...
  function exportCsv(){
    // export ignores 'category' on purpose (exports full dataset under current meta filters)
    const url = `/api/admin/export?${qs({range, team, shift, location})}`;
    window.open(url, '_blank');
  }

  useEffect(()=>{ load(); }, [range, team, shift, location, category]);

  // ====== Derived datasets for charts ======
  // (1) Per-question bars
  const likertBars = useMemo(() =>
    (data?.summary?.likertAvg || []).map((r:any)=>({ id:r.id, avg: Number((r.avg||0).toFixed(2)) })), [data]);

  const yesnoBars = useMemo(() =>
    (data?.summary?.yesnoRate || []).map((r:any)=>({ id:r.id, rate: Math.round((r.rate||0)*100) })), [data]);

  // (2) Trend chart: convert yesRate (0..1) -> yesRatePct (0..100) so the <Line> doesn’t need a formatter prop
  const trendData = useMemo(() =>
    (data?.trend || []).map((d:any) => ({ ...d, yesRatePct: Math.round(((d.yesRate || 0) * 100)) })), [data]);

  // (3) Month-to-date vs Prior-month comparisons by category
  const compareLikert = useMemo(()=>{
    const m = (data?.mtd?.categories || []).map((c:any)=>({ cat: c.cat, MTD: Number((c.likertAvg||0).toFixed(2)) }));
    const p = (data?.prior?.categories || []).reduce((acc:any, c:any)=> (acc[c.cat] = Number((c.likertAvg||0).toFixed(2)), acc), {} as Record<string,number>);
  ...
  const compareYes = useMemo(()=>{
    ...
    const p = (data?.prior?.categories || []).reduce((acc:any, c:any)=> (acc[c.cat] = Math.round((c.yesRate||0)*100), acc), {} as Record<string,number>);
    return m.map((row:any)=>({ cat: row.cat, MTD: row.MTD, Prior: p[row.cat] ?? 0 }));
  }, [data]);
